...
type PromptChainWithRelations = LlmPromptChain & {
  caption_request?: {
    id: number
...
export default function LlmPromptChainsPage() {
  const [items,   setItems]   = useState<PromptChainWithRelations[]>([])
  ...
  const fetchItems = useCallback(async (p: number) => {
    ...
    try {
      ...
      const res    = await fetch(`/api/llm-prompt-chains?${params}`)
  ...
  return (
    <div className="px-6 py-8 space-y-6 max-w-7xl mx-auto">
      <div>
        <h2 className="text-2xl font-bold text-gray-900">LLM Prompt Chains</h2>
        <p className="text-sm text-gray-500 mt-0.5">
          {loading ? 'Loading…' : `${total.toLocaleString()} chain${total !== 1 ? 's' : ''} total`}
        </p>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200">
                ...
                <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Caption Request ID</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                ...
                <tr>
                  <td colSpan={5} className="px-4 py-12 text-center text-gray-400">No prompt chains found</td>
                </tr>
              ) : (
                items.map(item => (
                  <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      {item.caption_request?.image?.url ? (
                        <img src={item.caption_request.image.url} alt="" className="w-12 h-12 object-cover rounded-lg bg-gray-100" />
                      ) : (
                    ...
                    <td className="px-4 py-3 text-gray-600 text-xs">
                      {item.caption_request?.profile?.email ?? (
                        item.caption_request?.profile?.first_name
                          ? `${item.caption_request.profile.first_name} ${item.caption_request.profile.last_name ?? ''}`.trim()
